 ...
 * first-order spring whose rate falls with how deep in the crowd it stands: the
 * front line is nearly pinned, the ranks behind it arrive a beat later, and a
 * turn travels backward down the column. Units lean into the direction they
 * are sliding, which is the cheapest thing that reads as weight on a crowd
 * whose instances carry a yaw and nothing else.
 *
...
export class UnitFlock {
  ...
  /**
   * Moves the unit at `index`, standing `depth` metres behind the crowd's
   * anchor, toward `(targetX, targetZ)`.
   *
   * Depth in metres rather than the slot's row number (D42): the sim's rows are
   * 0.29 m apart at a handful of units and 0.175 m apart at five hundred, so the
   * same row index is a different place in the crowd at every squad size, and a
   * 77-rank column tuned by row number either pins its whole visible half or
   * turns to soup at twenty units.
   *
   ...
   */
  follow(index: number, depth: number, targetX: number, targetZ: number, dt: number): void {
    ...
    // Toward the floor rather than onto it, so a turn keeps travelling past the
    // ranks a ramp would have flattened into one block (`UNIT_FOLLOW_DEPTH`).
    const rate =
      UNIT_FOLLOW_BACK +
      (UNIT_FOLLOW_FRONT - UNIT_FOLLOW_BACK) / (1 + Math.max(0, depth) / UNIT_FOLLOW_DEPTH);
